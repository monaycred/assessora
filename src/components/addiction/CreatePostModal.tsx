...
export function CreatePostModal({ trackerId, onSuccess }: Props) {
  ...
  <Modal open={open} onClose={()=>setOpen(false)} title="Criar publicação"><form onSubmit={submit} className="space-y-5">
    <div><Label>Onde publicar?</Label><div className="mt-2 grid gap-2 sm:grid-cols-2">
      ...
      <button type="button" onClick={()=>{setAudience('groups');setSelected(groups.map(g=>g.id))}} className={`rounded-2xl border-2 p-3 text-left ${audience==='groups'?'border-purple-500 bg-purple-50':'border-slate-200'}`}><Users className="mb-1 h-5 w-5 text-purple-600"/><b>Meus grupos</b><small className="block text-slate-600">Todos começam selecionados</small></button>
    </div></div>
