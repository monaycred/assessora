...
export default function ModuleAccessManager({ profileId, initialEnabled }: {
  ...
  const [enabled, setEnabled] = useState(initialEnabled.filter((key) => key !== 'addiction'));
  ...
  return (
    <details className="mt-3 border-t border-dark-700/50 pt-3">
      <summary className="cursor-pointer text-xs font-medium text-primary-400">Controle de Vícios gratuito · módulos adicionais ({enabled.length})</summary>
